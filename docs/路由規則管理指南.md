...
## 1. 整體資料流
1. **批次上傳**：`debug_tools/batch_upload` 工具把音訊及其 `info_features` 寫入指定的 MongoDB 實例（欄位範本見 `debug_tools/batch_upload/CHANGES_SUMMARY.md`）。  
2. **任務偵測**：`core/state_management/services/task_scheduler.py` 監看每個啟用的 MongoDB 實例，輪詢 `info_features.upload_time`，一旦偵測到新記錄即讀取其 `info_features`。  
3. **匹配規則**：調度器呼叫 `RoutingRule.find_matching_rules()`（來源：`core/state_management/models/routing_rule.py`）依優先級比對 `conditions`。  
4. **建立任務**：對於每個命中的 `actions`，調度器組合任務 payload（分析方法、參數 config、目標 MongoDB 實例等）並以 `analysis.<analysis_method_id>.<priority>` 的 routing key 發送到 RabbitMQ（`core/state_management/utils/rabbitmq_handler.py`）。  
5. **分析節點消費**：分析服務（例如 `a_sub_system/analysis_service_v2`）訂閱 `analysis.#` routing key，接收任務後依 `config_id` 讀取參數並執行實際分析。

---

## 2. 必備前置設定
- **MongoDB 實例**：於 Web UI「MongoDB 實例」或 `MongoDBInstance` API 建立連線資訊，確保資料來源與分析節點能共用。可在 `core/state_management/views/instance_views.py` 找到對應表單欄位。  
...
| `priority` | 整數，數值越大代表越先匹配；同時會作為 RabbitMQ routing key 的末段。 |
| `conditions` | JSON 物件，用於比對 `info_features`。支援單值、陣列與複雜運算子。 |
...
| `mongodb_instance` | （可選）指定分析節點要連線的 MongoDB 實例 ID；預設為寫入資料的來源實例。 |
...
| 症狀 | 檢查項 |
| --- | --- |
| 新資料未觸發任務 | 確認 `TaskScheduler` 是否正在執行、MongoDB 實例已啟用、`info_features.upload_time` 是否存在。 |
...
5. 分析節點驗收結果後，再推廣至正式環境，必要時透過 `priority` 控制併行規則的執行順序。
